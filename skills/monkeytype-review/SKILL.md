...
- **Mistakes**: each typo, found by aligning the first attempt (`typed`, before corrections) with the target word and placing keys on the standard touch-typing finger map: swapped letters, wrong key (neighbour key, same finger, mirror = same finger on the other hand, other), Shift error, missing, extra or doubled letter, early space, unfinished word. An extra key marked `(neighbour key)` sits next to the letter that was wanted. A slip usually shifts the rest of the attempt until it's noticed, so the script reads that as one extra or missing key and ignores the untyped end of a corrected word.
