...
Route each learning to its correct destination — not everything belongs in CLAUDE.md:
- **User preferences and working style** → the memory directory, one fact per file with `name` / `description` / `metadata.type` frontmatter, plus a one-line pointer in `MEMORY.md`. Use type `user` for who they are, `feedback` for guidance on how to work (include the why and how to apply it).
- **Repo-wide conventions** → CLAUDE.md. Reserve it for things that apply to anyone touching the code: build and test commands, project layout, house style.
...
1. Memory files for preferences; CLAUDE.md updates for repo-wide conventions
