...
function saveFile(filename, content) {
    ...
    // api.Front.showPopup("Downloading " + filename);

    // Create a synthetic download link
...
api.mapkey(";fwew", "Extract empWorkDialog info", function() {
    const root = document.querySelector("#empWorkDialog");
    if (!root) {
        api.Front.showPopup("empWorkDialog not found");
        return;
    }

    // Extract and normalize date
    const rawDate = root.querySelector("#empWorkDate")?.textContent.trim() || "";

    // Convert "2025年12月2日火曜日" to "2025-12-02"
    ...
    // Real work total
    const realWork = root.querySelector("#empWorkRealTime")?.textContent.trim() || "";

    // Task extraction
    const rows = [...root.querySelectorAll("#empWorkTableBody > tr")];
    ...
    const tasks = rows.map((tr, idx) => {
        ...
        const [code, name] = hidden.value.split("|", 2);

        // Determine mode by active toggle label
        let mode = null;
        const clockLabel   = tr.querySelector('[id^="btnClock2Label"]');
        const percentLabel = tr.querySelector('[id^="btnPercentLabel"]');
        const rangeLabel   = tr.querySelector('[id^="btnTimeRangeLabel"]');

        if (clockLabel?.classList.contains("toggle_switch_label_active")) mode = "1";
        if (percentLabel?.classList.contains("toggle_switch_label_active")) mode = "2";
        if (rangeLabel?.classList.contains("toggle_switch_label_active")) mode = "0";

        // Extract final computed time
        let timeSpent = "";
        const label = tr.querySelector('[id^="empTimeLabel"]');
        if (label) timeSpent = label.textContent.trim();

        if (!timeSpent || timeSpent === "-" || timeSpent.length === 0) {
            timeSpent = "0:00";
        }
    ...
    const payload = {
        date: isoDate,
        real_work_time: realWork.replace("実労働時間：", "").trim(),
        tasks: tasks
    };
    const json = JSON.stringify(payload, null, 2);
    saveFile(`teamspirit_working_time-${isoDate}.json`, json);
});
...
api.mapkey('gh', 'Open History Sidebar', function() {
    window.location.href = 'chrome://browser/content/places/historySidebar.xhtml';
});
